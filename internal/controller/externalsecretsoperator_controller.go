...
func (r *ExternalSecretsOperatorReconciler) createObjectManager(obj *apiv1alpha1.ExternalSecretsOperator, pc *apiv1alpha1.ProviderConfig, clusters clusteraccess.ClusterContext) (objectmanager.Manager, error) {
	...
	err = r.manageImagePullSecrets(platformCluster, mcpCluster, helmValues, externalSecretsNamespace)
	if err != nil {
		return nil, err
	}
...
func (r *ExternalSecretsOperatorReconciler) manageImagePullSecrets(platformCluster, mcpCluster objectmanager.Cluster, helmValues *externalsecrets.HelmValues, externalSecretsNamespace string) error {
	for _, imagePullSecret := range helmValues.Global.ImagePullSecrets {
		...
		if err != nil {
			return fmt.Errorf("configuring imagepullsecret: %w", err)
		}
	}
	return nil
}
...
func (r *ExternalSecretsOperatorReconciler) manageChartPullSecret(platformCluster objectmanager.Cluster, esoVersion apiv1alpha1.RequestedVersion, tenantNamespace string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("configuring chartpullsecret: %w", err)
	}
